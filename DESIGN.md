...
## Widget Grid — Emphasis Model (v1.2)
...
2. **Status band** — emerald (good/on-track), amber (at-risk/drag), rose (behind), neutral (pure status, no target). Computed reactively from live data on each refresh.

**Reactive emphasis** (chosen over manual/pinned emphasis):
...
**Tradeoff accepted:** the layout shifts week to week (not screenshot-stable) in exchange for a dashboard that always surfaces the two things that are off — the right behavior for a Monday-morning EM glance.
...
**Render templates** (4, from the v1.2 spec): `verdict_card` (rich, text-heavy), `scoreboard` (numeric headline + delta), `list` (ranked deeplinked rows), `single_stat` (one number + verdict). All four obey the size→treatment and status→volume rules above.
...
- **Duration:** micro(50–100ms) · short(150–250ms) · medium(250–400ms). No long/choreographed animation — this is a glance, not an experience. Reactive reorder uses a short move transition so cards don't jump.
